...
class SessionsApi {
  ...
  Future<http.Response> createSessionWithHttpInfo({ CreateSessionRequest? createSessionRequest, }) async {
  ...
  Future<http.Response> createSessionTokenWithHttpInfo(String sessionId, { CreateSessionTokenRequest? createSessionTokenRequest, }) async {
    // ignore: prefer_const_declarations
    final path = r'/sessions/{session_id}/tokens'
  ...
  Future<CreateSessionToken200Response?> createSessionToken(String sessionId, { CreateSessionTokenRequest? createSessionTokenRequest, }) async {
    final response = await createSessionTokenWithHttpInfo(sessionId,  createSessionTokenRequest: createSessionTokenRequest, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'CreateSessionToken200Response',) as CreateSessionToken200Response;
  ...
  Future<http.Response> createSessionTokenFromTemplateWithHttpInfo(String sessionId, String templateName, { CreateSessionTokenFromTemplateRequest? createSessionTokenFromTemplateRequest, }) async {
    // ignore: prefer_const_declarations
  ...
  Future<CreateSessionToken200Response?> createSessionTokenFromTemplate(String sessionId, String templateName, { CreateSessionTokenFromTemplateRequest? createSessionTokenFromTemplateRequest, }) async {
    final response = await createSessionTokenFromTemplateWithHttpInfo(sessionId, templateName,  createSessionTokenFromTemplateRequest: createSessionTokenFromTemplateRequest, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  /// * [String] sessionId (required):
  ///   The ID of the session
  Future<http.Response> getSessionWithHttpInfo(String sessionId,) async {
    // ignore: prefer_const_declarations
    final path = r'/sessions/{session_id}'
      .replaceAll('{session_id}', sessionId);
  ...
  Future<Session?> getSession(String sessionId,) async {
    final response = await getSessionWithHttpInfo(sessionId,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  Future<http.Response> getSessionListWithHttpInfo({ String? clientId, String? userId, String? status, bool? paginated, int? limit, int? offset, }) async {
    // ignore: prefer_const_declarations
    final path = r'/sessions';
  ...
  Future<List<Session>?> getSessionList({ String? clientId, String? userId, String? status, bool? paginated, int? limit, int? offset, }) async {
    final response = await getSessionListWithHttpInfo( clientId: clientId, userId: userId, status: status, paginated: paginated, limit: limit, offset: offset, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  /// * [String] sessionId (required):
  ///   The ID of the session
  Future<http.Response> revokeSessionWithHttpInfo(String sessionId,) async {
    // ignore: prefer_const_declarations
    final path = r'/sessions/{session_id}/revoke'
      .replaceAll('{session_id}', sessionId);
  ...
  Future<Session?> revokeSession(String sessionId,) async {
    final response = await revokeSessionWithHttpInfo(sessionId,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  ///   Parameters.
  Future<http.Response> verifySessionWithHttpInfo(String sessionId, { VerifySessionRequest? verifySessionRequest, }) async {
    // ignore: prefer_const_declarations
    final path = r'/sessions/{session_id}/verify'
      .replaceAll('{session_id}', sessionId);
  ...
  Future<Session?> verifySession(String sessionId, { VerifySessionRequest? verifySessionRequest, }) async {
    final response = await verifySessionWithHttpInfo(sessionId,  verifySessionRequest: verifySessionRequest, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
