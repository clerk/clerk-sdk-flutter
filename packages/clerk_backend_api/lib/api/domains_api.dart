...
class DomainsApi {
  ...
  Future<http.Response> addDomainWithHttpInfo({ AddDomainRequest? addDomainRequest, }) async {
  ...
  Future<Domain?> addDomain({ AddDomainRequest? addDomainRequest, }) async {
    final response = await addDomainWithHttpInfo( addDomainRequest: addDomainRequest, );
  ...
  Future<http.Response> deleteDomainWithHttpInfo(String domainId,) async {
    // ignore: prefer_const_declarations
    final path = r'/domains/{domain_id}'
  ...
  Future<DeletedObject?> deleteDomain(String domainId,) async {
    final response = await deleteDomainWithHttpInfo(domainId,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  Future<Domains?> listDomains() async {
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'Domains',) as Domains;
  ...
  Future<http.Response> updateDomainWithHttpInfo(String domainId, UpdateDomainRequest updateDomainRequest,) async {
    // ignore: prefer_const_declarations
    final path = r'/domains/{domain_id}'
      .replaceAll('{domain_id}', domainId);
  ...
  Future<Domain?> updateDomain(String domainId, UpdateDomainRequest updateDomainRequest,) async {
    final response = await updateDomainWithHttpInfo(domainId, updateDomainRequest,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
