...
class ClerkErrors {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is ClerkErrors &&
  ...
  @override
  String toString() => 'ClerkErrors[errors=$errors, meta=$meta]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ClerkErrors[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ClerkErrors[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<ClerkErrors> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<ClerkErrors>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<ClerkErrors>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = ClerkErrors.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
