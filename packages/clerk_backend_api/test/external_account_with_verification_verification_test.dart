//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:clerk_backend_api/api.dart';
import 'package:test/test.dart';

// tests for ExternalAccountWithVerificationVerification
void main() {
  // final instance = ExternalAccountWithVerificationVerification();

  group('test ExternalAccountWithVerificationVerification', () {
    // String object
    test('to test the property `object`', () async {
      // TODO
    });

    // String status
    test('to test the property `status`', () async {
      // TODO
    });

    // String strategy
    test('to test the property `strategy`', () async {
      // TODO
    });

    // String externalVerificationRedirectUrl
    test('to test the property `externalVerificationRedirectUrl`', () async {
      // TODO
    });

    // VerificationFromOauthError error
    test('to test the property `error`', () async {
      // TODO
    });

    // int expireAt
    test('to test the property `expireAt`', () async {
      // TODO
    });

    // int attempts
    test('to test the property `attempts`', () async {
      // TODO
    });

    // String verifiedAtClient
    test('to test the property `verifiedAtClient`', () async {
      // TODO
    });
  });
}
