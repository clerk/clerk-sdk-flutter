...
class IdentificationLink {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is IdentificationLink &&
    other.type == type &&
    other.id == id;
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "IdentificationLink[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "IdentificationLink[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<IdentificationLink> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<IdentificationLink>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<IdentificationLink>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = IdentificationLink.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
