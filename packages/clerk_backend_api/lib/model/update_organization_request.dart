...
class UpdateOrganizationRequest {
  ...
  @override
  String toString() => 'UpdateOrganizationRequest[publicMetadata=$publicMetadata, privateMetadata=$privateMetadata, name=$name, slug=$slug, maxAllowedMemberships=$maxAllowedMemberships, adminDeleteEnabled=$adminDeleteEnabled, createdAt=$createdAt]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "UpdateOrganizationRequest[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "UpdateOrganizationRequest[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<UpdateOrganizationRequest> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<UpdateOrganizationRequest>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<UpdateOrganizationRequest>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = UpdateOrganizationRequest.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
