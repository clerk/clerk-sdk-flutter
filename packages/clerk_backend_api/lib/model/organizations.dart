...
class Organizations {
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "Organizations[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "Organizations[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<Organizations> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<Organizations>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<Organizations>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = Organizations.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
