...
class OAuthApplications {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is OAuthApplications &&
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "OAuthApplications[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "OAuthApplications[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<OAuthApplications> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<OAuthApplications>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<OAuthApplications>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = OAuthApplications.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
