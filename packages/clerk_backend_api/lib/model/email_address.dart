...
class EmailAddress {
  ...
  @override
  String toString() => 'EmailAddress[id=$id, object=$object, emailAddress=$emailAddress, reserved=$reserved, verification=$verification, linkedTo=$linkedTo, matchesSsoConnection=$matchesSsoConnection, createdAt=$createdAt, updatedAt=$updatedAt]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "EmailAddress[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "EmailAddress[$key]" has a null value in JSON.');
        });
        return true;
      }());
      ...
      return EmailAddress(
        ...
        matchesSsoConnection: mapValueOfType<bool>(json, r'matches_sso_connection'),
  ...
  static List<EmailAddress> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<EmailAddress>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<EmailAddress>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = EmailAddress.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
...
class EmailAddressObjectEnum {
  ...
  static EmailAddressObjectEnum? fromJson(dynamic value) => EmailAddressObjectEnumTypeTransformer().decode(value);

  static List<EmailAddressObjectEnum> listFromJson(dynamic json, {bool growable = false,}) {
...
class EmailAddressObjectEnumTypeTransformer {
  factory EmailAddressObjectEnumTypeTransformer() => _instance ??= const EmailAddressObjectEnumTypeTransformer._();
  ...
  EmailAddressObjectEnum? decode(dynamic data, {bool allowNull = true}) {
    if (data != null) {
      switch (data) {
        case r'email_address': return EmailAddressObjectEnum.emailAddress;
        default:
          if (!allowNull) {
