...
class SAMLConnections {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is SAMLConnections &&
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "SAMLConnections[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "SAMLConnections[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<SAMLConnections> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<SAMLConnections>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<SAMLConnections>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = SAMLConnections.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
