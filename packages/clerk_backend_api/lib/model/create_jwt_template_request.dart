...
class CreateJWTTemplateRequest {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is CreateJWTTemplateRequest &&
    other.name == name &&
    other.claims == claims &&
  ...
  @override
  String toString() => 'CreateJWTTemplateRequest[name=$name, claims=$claims, lifetime=$lifetime, allowedClockSkew=$allowedClockSkew, customSigningKey=$customSigningKey, signingAlgorithm=$signingAlgorithm, signingKey=$signingKey]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "CreateJWTTemplateRequest[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "CreateJWTTemplateRequest[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<CreateJWTTemplateRequest> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<CreateJWTTemplateRequest>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<CreateJWTTemplateRequest>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = CreateJWTTemplateRequest.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
