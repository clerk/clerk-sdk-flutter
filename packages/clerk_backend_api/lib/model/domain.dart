...
class Domain {
  ...
  @override
  String toString() => 'Domain[object=$object, id=$id, name=$name, isSatellite=$isSatellite, frontendApiUrl=$frontendApiUrl, accountsPortalUrl=$accountsPortalUrl, proxyUrl=$proxyUrl, developmentOrigin=$developmentOrigin, cnameTargets=$cnameTargets]';
...
class DomainObjectEnum {
  ...
  static DomainObjectEnum? fromJson(dynamic value) => DomainObjectEnumTypeTransformer().decode(value);

  static List<DomainObjectEnum> listFromJson(dynamic json, {bool growable = false,}) {
...
class DomainObjectEnumTypeTransformer {
  factory DomainObjectEnumTypeTransformer() => _instance ??= const DomainObjectEnumTypeTransformer._();
  ...
  DomainObjectEnum? decode(dynamic data, {bool allowNull = true}) {
    if (data != null) {
      switch (data) {
        case r'domain': return DomainObjectEnum.domain;
        default:
          if (!allowNull) {
