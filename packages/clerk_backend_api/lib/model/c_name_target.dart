...
class CNameTarget {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is CNameTarget &&
    other.host == host &&
    other.value == value &&
  ...
  @override
  String toString() => 'CNameTarget[host=$host, value=$value, required_=$required_]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "CNameTarget[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "CNameTarget[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<CNameTarget> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<CNameTarget>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<CNameTarget>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = CNameTarget.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
