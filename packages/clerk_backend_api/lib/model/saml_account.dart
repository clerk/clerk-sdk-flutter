...
class SAMLAccount {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is SAMLAccount &&
    other.id == id &&
    other.object == object &&
  ...
  @override
  String toString() => 'SAMLAccount[id=$id, object=$object, provider=$provider, active=$active, emailAddress=$emailAddress, firstName=$firstName, lastName=$lastName, providerUserId=$providerUserId, publicMetadata=$publicMetadata, verification=$verification, samlConnection=$samlConnection]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "SAMLAccount[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "SAMLAccount[$key]" has a null value in JSON.');
        });
        return true;
      }());
      ...
      return SAMLAccount(
        ...
        samlConnection: SAMLAccountSamlConnection.fromJson(json[r'saml_connection']),
      );
    }
    return null;
  }

  static List<SAMLAccount> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<SAMLAccount>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<SAMLAccount>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = SAMLAccount.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
...
class SAMLAccountObjectEnum {
  ...
  static SAMLAccountObjectEnum? fromJson(dynamic value) => SAMLAccountObjectEnumTypeTransformer().decode(value);

  static List<SAMLAccountObjectEnum> listFromJson(dynamic json, {bool growable = false,}) {
...
class SAMLAccountObjectEnumTypeTransformer {
  factory SAMLAccountObjectEnumTypeTransformer() => _instance ??= const SAMLAccountObjectEnumTypeTransformer._();
  ...
  SAMLAccountObjectEnum? decode(dynamic data, {bool allowNull = true}) {
    if (data != null) {
      switch (data) {
        case r'saml_account': return SAMLAccountObjectEnum.samlAccount;
        default:
          if (!allowNull) {
