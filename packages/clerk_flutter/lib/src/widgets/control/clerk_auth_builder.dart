...
class _ClerkAuthBuilderState extends State<ClerkAuthBuilder>
  ...
  Widget build(BuildContext context) {
    ...
    if (auth.client.user is clerk.User) {
      if (widget.signedInBuilder case final builder?) {
        return builder(context, auth);
      }
    } else {
      if (widget.signingInBuilder case final builder? when auth.isSigningIn) {
        return builder(context, auth);
      }

      if (widget.signingUpBuilder case final builder? when auth.isSigningUp) {
        return builder(context, auth);
      }

      if (widget.signedOutBuilder case final builder?) {
        return builder(context, auth);
      }
    }
