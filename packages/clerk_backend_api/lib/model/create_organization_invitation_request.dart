...
class CreateOrganizationInvitationRequest {
  ...
  @override
  String toString() => 'CreateOrganizationInvitationRequest[emailAddress=$emailAddress, inviterUserId=$inviterUserId, role=$role, publicMetadata=$publicMetadata, privateMetadata=$privateMetadata, redirectUrl=$redirectUrl, expiresInDays=$expiresInDays]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "CreateOrganizationInvitationRequest[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "CreateOrganizationInvitationRequest[$key]" has a null value in JSON.');
        });
        return true;
      }());
