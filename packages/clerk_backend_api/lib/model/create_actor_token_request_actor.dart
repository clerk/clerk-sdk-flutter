...
class CreateActorTokenRequestActor {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is CreateActorTokenRequestActor &&
    other.sub == sub;
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "CreateActorTokenRequestActor[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "CreateActorTokenRequestActor[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<CreateActorTokenRequestActor> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<CreateActorTokenRequestActor>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<CreateActorTokenRequestActor>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = CreateActorTokenRequestActor.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
