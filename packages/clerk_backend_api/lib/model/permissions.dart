...
class Permissions {
  ...
  @override
  String toString() => 'Permissions[data=$data, totalCount=$totalCount]';
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "Permissions[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "Permissions[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return Permissions(
        data: Permission.listFromJson(json[r'data']),
  ...
  static List<Permissions> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
  ...
  static Map<String, List<Permissions>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = Permissions.listFromJson(
          entry.value,
          growable: growable,
        );
      }
