...
class InvitationsApi {
  ...
  ///   Required parameters
  Future<http.Response> createBulkInvitationsWithHttpInfo({ List<CreateBulkInvitationsRequestInner>? createBulkInvitationsRequestInner, }) async {
  ...
  Future<List<Invitation>?> createBulkInvitations({ List<CreateBulkInvitationsRequestInner>? createBulkInvitationsRequestInner, }) async {
    final response = await createBulkInvitationsWithHttpInfo( createBulkInvitationsRequestInner: createBulkInvitationsRequestInner, );
  ...
  ///   Required parameters
  Future<http.Response> createInvitationWithHttpInfo({ CreateInvitationRequest? createInvitationRequest, }) async {
    // ignore: prefer_const_declarations
  ...
  Future<Invitation?> createInvitation({ CreateInvitationRequest? createInvitationRequest, }) async {
    final response = await createInvitationWithHttpInfo( createInvitationRequest: createInvitationRequest, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  Future<http.Response> listInvitationsWithHttpInfo({ String? status, String? query, String? orderBy, bool? paginated, int? limit, int? offset, }) async {
    // ignore: prefer_const_declarations
    final path = r'/invitations';
  ...
  Future<List<Invitation>?> listInvitations({ String? status, String? query, String? orderBy, bool? paginated, int? limit, int? offset, }) async {
    final response = await listInvitationsWithHttpInfo( status: status, query: query, orderBy: orderBy, paginated: paginated, limit: limit, offset: offset, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  Future<http.Response> revokeInvitationWithHttpInfo(String invitationId,) async {
    // ignore: prefer_const_declarations
  ...
  Future<RevokeInvitation200Response?> revokeInvitation(String invitationId,) async {
    final response = await revokeInvitationWithHttpInfo(invitationId,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'RevokeInvitation200Response',) as RevokeInvitation200Response;
