...
class AddDomainRequest {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is AddDomainRequest &&
    other.name == name &&
  ...
  @override
  String toString() => 'AddDomainRequest[name=$name, isSatellite=$isSatellite, proxyUrl=$proxyUrl]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "AddDomainRequest[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "AddDomainRequest[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<AddDomainRequest> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<AddDomainRequest>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<AddDomainRequest>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = AddDomainRequest.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
