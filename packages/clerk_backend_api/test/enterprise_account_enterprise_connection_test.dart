//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:clerk_backend_api/api.dart';
import 'package:test/test.dart';

// tests for EnterpriseAccountEnterpriseConnection
void main() {
  // final instance = EnterpriseAccountEnterpriseConnection();

  group('test EnterpriseAccountEnterpriseConnection', () {
    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // String protocol
    test('to test the property `protocol`', () async {
      // TODO
    });

    // String provider
    test('to test the property `provider`', () async {
      // TODO
    });

    // String name
    test('to test the property `name`', () async {
      // TODO
    });

    // String logoPublicUrl
    test('to test the property `logoPublicUrl`', () async {
      // TODO
    });

    // String domain
    test('to test the property `domain`', () async {
      // TODO
    });

    // List<String> domains (default value: const [])
    test('to test the property `domains`', () async {
      // TODO
    });

    // bool active
    test('to test the property `active`', () async {
      // TODO
    });

    // bool syncUserAttributes
    test('to test the property `syncUserAttributes`', () async {
      // TODO
    });

    // bool allowSubdomains
    test('to test the property `allowSubdomains`', () async {
      // TODO
    });

    // bool allowIdpInitiated
    test('to test the property `allowIdpInitiated`', () async {
      // TODO
    });

    // bool disableAdditionalIdentifications
    test('to test the property `disableAdditionalIdentifications`', () async {
      // TODO
    });

    // Unix timestamp of creation.
    // int createdAt
    test('to test the property `createdAt`', () async {
      // TODO
    });

    // Unix timestamp of last update.
    // int updatedAt
    test('to test the property `updatedAt`', () async {
      // TODO
    });
  });
}
