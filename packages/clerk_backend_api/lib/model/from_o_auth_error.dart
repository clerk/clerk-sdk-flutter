...
class FromOAuthError {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is FromOAuthError &&
  ...
  @override
  String toString() => 'FromOAuthError[message=$message, longMessage=$longMessage, code=$code, meta=$meta, clerkTraceId=$clerkTraceId]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "FromOAuthError[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "FromOAuthError[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<FromOAuthError> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<FromOAuthError>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<FromOAuthError>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = FromOAuthError.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
