//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:openapi/api.dart';
import 'package:test/test.dart';

// tests for OrganizationInvitationWithPublicOrganizationData
void main() {
  // final instance = OrganizationInvitationWithPublicOrganizationData();

  group('test OrganizationInvitationWithPublicOrganizationData', () {
    // String representing the object's type. Objects of the same type share the same value. 
    // String object
    test('to test the property `object`', () async {
      // TODO
    });

    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // String emailAddress
    test('to test the property `emailAddress`', () async {
      // TODO
    });

    // String role
    test('to test the property `role`', () async {
      // TODO
    });

    // String roleName
    test('to test the property `roleName`', () async {
      // TODO
    });

    // String organizationId
    test('to test the property `organizationId`', () async {
      // TODO
    });

    // String status
    test('to test the property `status`', () async {
      // TODO
    });

    // Map<String, Object> publicMetadata (default value: const {})
    test('to test the property `publicMetadata`', () async {
      // TODO
    });

    // Map<String, Object> privateMetadata (default value: const {})
    test('to test the property `privateMetadata`', () async {
      // TODO
    });

    // String url
    test('to test the property `url`', () async {
      // TODO
    });

    // Unix timestamp of expiration.
    // int expiresAt
    test('to test the property `expiresAt`', () async {
      // TODO
    });

    // Unix timestamp of creation.
    // int createdAt
    test('to test the property `createdAt`', () async {
      // TODO
    });

    // Unix timestamp of last update.
    // int updatedAt
    test('to test the property `updatedAt`', () async {
      // TODO
    });

    // OrganizationInvitationPublicOrganizationData publicOrganizationData
    test('to test the property `publicOrganizationData`', () async {
      // TODO
    });


  });

}
