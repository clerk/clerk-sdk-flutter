...
class EnterpriseConnection {
  /// Returns a new [EnterpriseConnection] instance.
  ...
  @override
  String toString() =>
      'EnterpriseConnection[id=$id, protocol=$protocol, provider=$provider, name=$name, logoPublicUrl=$logoPublicUrl, domain=$domain, domains=$domains, active=$active, syncUserAttributes=$syncUserAttributes, allowSubdomains=$allowSubdomains, allowIdpInitiated=$allowIdpInitiated, disableAdditionalIdentifications=$disableAdditionalIdentifications, createdAt=$createdAt, updatedAt=$updatedAt]';
  ...
  /// Returns a new [EnterpriseConnection] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static EnterpriseConnection? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "EnterpriseConnection[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "EnterpriseConnection[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<EnterpriseConnection> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    final result = <EnterpriseConnection>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = EnterpriseConnection.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, EnterpriseConnection> mapFromJson(dynamic json) {
    final map = <String, EnterpriseConnection>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = EnterpriseConnection.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of EnterpriseConnection-objects as value to a dart map
  static Map<String, List<EnterpriseConnection>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    final map = <String, List<EnterpriseConnection>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = EnterpriseConnection.listFromJson(
          entry.value,
          growable: growable,
        );
      }
