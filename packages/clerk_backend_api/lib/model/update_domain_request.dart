...
class UpdateDomainRequest {
  ...
  @override
  String toString() => 'UpdateDomainRequest[name=$name, proxyUrl=$proxyUrl, isSecondary=$isSecondary]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "UpdateDomainRequest[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "UpdateDomainRequest[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<UpdateDomainRequest> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<UpdateDomainRequest>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<UpdateDomainRequest>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = UpdateDomainRequest.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
